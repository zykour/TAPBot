...
namespace TAPBot
{
    class WeeklyAction : BotAction
    {
        public WeeklyAction(Func<ApiService> CreateApiService) : base(CreateApiService) { }

        protected override BotContext ProduceOutgoingMessage(BotContext botContext)
        {
            BotContext sendContext = botContext.Clone();

            List<Listing> deals = apiService.GetWeeklyDeals().ToList();
        ...
        {
            if (chatInput.CompareTo("!weekly") == 0 ||
                 chatInput.CompareTo("/weekly") == 0 ||
                 chatInput.CompareTo("!weeklies") == 0 ||
                 chatInput.CompareTo("/weeklies") == 0 ||
                 chatInput.CompareTo("!weeklydeals") == 0 ||
                 chatInput.CompareTo("/weeklydeals") == 0)
            {
                return true;
            }
