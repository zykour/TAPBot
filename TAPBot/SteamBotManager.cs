...
namespace TAPBot
{
    public class SteamBotManager
    {
        ...
        private bool isRunningSuccessfully;
        private string apiUrl, apiLogin, apiPassword;
        
        public SteamBotManager(){}

        public bool InitializeAndRun(string username, string password, string apiUrl, string apiLogin, string apiPassword)
        {
            InitializeBot(username, password, apiUrl, apiLogin, apiPassword);
            return Run();
        }

        public void InitializeBot(string username, string password, string apiUrl, string apiLogin, string apiPassword)
        {
            ...
            ApiAuthorizer authorizer = new ApiAuthorizer(apiUrl, apiLogin, apiPassword);

            //edit
            commandFactory = new CommandFactory(steamFriends, authorizer);
        ...
        { 
            ...
            {
                callbackManager.RunWaitCallbacks(TimeSpan.FromSeconds(.3));
            }
        ...
        private void OnLoggedOn(SteamUser.LoggedOnCallback callback)
        {
            ...
            if (steamFriends.GetPersonaName().ToLower().Contains("unassigned"))
            {
                steamFriends.SetPersonaName("Demo Dan™");
            }
